...
void solve()
{
	...
	map< string, int> dp;
	vector<string> U{"A", "B", "C", "AB", "AC", "BC", "ABC"};
	for(auto &i : U) dp[i] = INF;
	for(int i = 0; i < n; ++i) {
		int p; string s;
		cin >> p >> s;
		sort(all(s));
	...
	dp["ABC"] = min(dp["ABC"] , dp["A"] + dp["BC"]);
	dp["ABC"] = min(dp["ABC"] , dp["B"] + dp["AC"]);
	dp["ABC"] = min(dp["ABC"] , dp["C"] + dp["AB"]);
	dp["ABC"] = min(dp["ABC"] , dp["A"] + dp["B"] + dp["C"]);
	
	cout << (dp["ABC"] < INF ? dp["ABC"] : -1) <<"\n";
}
