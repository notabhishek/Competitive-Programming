...
vector<int> a;
vector<int> b;
int dp[NAX][NAX]; //dp[personNumber][assign keys less than i]
int rec(int u, int id) {

	int &ret = dp[u][id];
	if(ret != -1) 
		return ret;
		
	ret = rec(u, id-1); // don't use the id key
	// take the id key
	int cost = abs(a[u-1]-b[id-1]) + abs(p-b[id-1]);
	ret = min(ret, max(rec(u-1,id-1),cost));
