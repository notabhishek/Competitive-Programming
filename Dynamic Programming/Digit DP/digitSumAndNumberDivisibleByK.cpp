...
const int MXLEN = 12;
const int MAXS = (MXLEN-1) * 9 + 1;
int dp[MXLEN][MAXS][MAXS][2];
int recur(string &num, int n, int rem, int x, bool tight, int k) {
	if(n == 0) 
		return rem == 0 && x == 0;
	int &ret = dp[n][rem][x][tight];
	if(ret != -1)
		return ret;
	int ub = tight ? num[num.length()-n]-'0' : 9;
	int answer = 0;
	for(int dig = 0; dig <= ub; ++dig) {
		answer += recur(num, n-1, (rem*10 + dig)%k, (x+dig)%k, tight & (dig == ub), k);
	}
	return ret = answer;
}
...
void solve()
{
	...
	string bs = to_string(b), as = to_string(a-1);
	int answer = recur(bs, bs.length(),0, 0, 1, k);
    if(a > 0) {
        memset(dp, -1, sizeof dp);
	    answer -= recur(as, as.length(), 0, 0, 1, k);
    }
	cout << answer << "\n";
}
