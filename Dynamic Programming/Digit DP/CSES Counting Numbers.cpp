...
const int MX_LEN = 20;
ll dp[MX_LEN][11][2][2];
ll rec(string &R, int n, int pdig, bool tight, bool fzero) {
	if(n == 0) return 1; 
	ll &ret = dp[n][pdig][tight][fzero];
	if(ret != -1) 
		return ret;
	...
	for(int dig = 0; dig <= ub; ++dig) {
		if(fzero || dig != pdig)
			answer += rec(R, n-1, dig, tight & (dig == ub), fzero & (dig == 0));
	}
	return ret = answer;
}
...
void solve()
{
	...
	ll answer = rec(R, R.length(), 0, 1, 1); 
	if(l >= 0) {
		memset(dp, -1, sizeof dp);
		answer -= rec(L, L.length(), 0, 1, 1);
	}
	cout << answer << "\n";
}
