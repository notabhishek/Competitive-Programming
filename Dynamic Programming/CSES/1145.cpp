...
void solve()
{
    ...
    // active[i] = end element of LIS of length i
    // It will be in ascending order due to our algo
	...
	for(int i = 1; i < a.size(); ++i) {
		if( a[i] <= active[0] ) 
			active[0] = a[i];
		else if( a[i] > active[ len - 1 ] ) 
			active[ len++ ] = a[i];
		else {
			//Value is in between binary search and find 
			// the rightmost list with last element < a[i] , extend that list 
			// so that you get a new list with lower end element than what 
			// was prev there
			int lo = 0 , hi = len - 1;
			int idx = 0;
			while (lo <= hi) {
