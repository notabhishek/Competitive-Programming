...
void solve()
{
	...
	dp[0][0] = pref[0][0] = 1;
	for(int i = 1; i <= N; ++i) // Any amount of people , 0 candies = 1 way
	{
			dp[i][0] = 1;
			pref[i][0] = 1;
	}
    ...
    for(int i = 1; i <= N; ++i ) {
    	for(int c = 1; c <= K; ++c) {
    		...
    		// dp[i][c]+= dp[i-1][c] + dp[i-1][c-1] + .... + dp[i-1][ c - min(a[i] , c ) ]
    		
    		// O(a[i]) calculation
    		// for(int take = 0; take <= min(a[i] , (ll)c); ++take ) {
    			// dp[i][c] = (dp[i][c] + dp[i-1][c - take]) % M;
    		// }
    		
    		// O(1) calculation using pref dp
    		int last = c - min(a[i] , (ll)c ) - 1;
    		dp[i][c] = (( pref[i-1][c] - (last>=0 ? pref[i-1][last] : 0 )) % M  + M) % M;
    		pref[i][c] = (pref[i][c-1] + dp[i][c]) % M;
    	}
