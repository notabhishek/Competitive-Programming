...
struct segTree
{
    ...
    void propogate(int v, int tl, int tr)
    {
        ...
        if (clazy[v] == true)
        {
            if (tl != tr)
            {
                clazy[2 * v] = 1 - clazy[2*v];
                clazy[2 * v + 1] = 1 - clazy[2*v + 1];
            }
	...
	int kthOne(int v, int tl, int tr , int idx) {
    	if(tree[v].sum < idx) 
    		return -1;
    	if(clazy[v])
    		propogate(v , tl , tr);
    	...
    	int tmid = (tl + tr)/2;
    	if(clazy[2*v])
    		propogate(2*v , tl , tmid);
    	if(clazy[2*v + 1])
    		propogate(2*v + 1 , tmid + 1 , tr);
    	if(idx > tree[2*v].sum) 
    		return kthOne(2*v + 1 , tmid + 1, tr , idx - tree[2*v].sum); 
    	else 
    		return kthOne(2*v , tl , tmid , idx);
    }
