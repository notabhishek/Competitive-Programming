...
// cnt = number of val left after cancelling 2 unequal elements, if cnt > 0, we may have a majority element
...
int getCnt(int l , int r, int v) {
	auto &a = pos[v];
	auto i = lower_bound(all(a), l) - a.begin();
	if(i == a.size()) return 0;
	auto j = upper_bound(all(a), r) - a.begin();
	return j - i;
}

struct node
{
    int val, cnt; // majority element, its count
    bool operator == (const node &a) {
        return a.cnt == cnt && a.val == val;
    }
...
//     	if(clazy[v]) 	
//     		propogate(v ,tl , tr);
//     	if(tree[v].cnt < k)  // does not exist
//     		return -1;
//     	if(tl == tr) 
//     		return tl;
//     	int tmid = (tl + tr)/2;
//     	if(tree[2*v].cnt < k) 
//     		return kthOne(2*v + 1 , tmid + 1 , tr , k - tree[2*v].cnt); 
//     	else 
...
void solve()
{
	...
	while(q--) {
		...
		int cnt = getCnt(l , r , ans.val);
		if(2*cnt > (len+1)) {
			/*
			majority exists, 221 221 221 pairs and 1 single pair
			2 * (cnt - 2x) <= len-3x + 1 , x pairs of 221
			2c - 4x <= len-3x+1
			2c-len-1 <= x
			*/
			int x = 2*cnt - len - 1;
			...
			// no majority, everything together
			cout << "1\n";
		}
	}
