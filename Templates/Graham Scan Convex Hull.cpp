struct point
{
	int x, y; // change to double if needed, also change set<pair<int,int>> in convext_hull
...
bool cw(point &a, point &b, point &c)
{
	...
	return area<=0; // remove = if you don't want collinear points
}
...
vector<point> convex_hull(vector<point> &v)
{
    ...
    for(auto &i : up) taken.insert({i.x, i.y});
    
	for(int i=down.size()-2;i>0;i--) // to remove duplicates in case of collinear points that come in both hulls
        if(!taken.count({down[i].x, down[i].y}))
		    up.push_back(down[i]);
