...
void solve()
{
	...
	// if initial parity != final parity  -> No , since we always add 2k
	if( abs((asum%2)) != abs((tsum %2)))
		f = 0;
	trace(f);
	
	// if bipartite then total addition to black nodes == addition to white nodes 
	// so    if sum0!=sum1 -> No
	if(bi && sum[0] != sum[1]) {
		f = 0;
	}
	
	// if not bipartite then there exists an edge having same color nodes , we can adjust sum0 / sum1 there
	trace(f);
	if(f) cout << "YES\n";
	else cout << "NO\n";
}
