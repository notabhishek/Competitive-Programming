...
const int N = 8;
int r8[8] = {1, 1, -1, -1, 2, 2, -2, -2};
int c8[8] = {2, -2, 2, -2, 1, -1, 1, -1};

vector<vector<int>> b;

int cntnbr(int r, int c) { // count empty neighbours 
	int nbr = 0;
	for(int i = 0; i < 8; ++i) {
		int tr = r + r8[i];
		int tc = c + c8[i];
		if(tr>=0 && tr<N && tc>=0 && tc<N && b[tr][tc]==-1) 
			++nbr;
	}
	return nbr;
}
...
bool dfs(int r, int c, int m){
	...
	// prioritize moving to nodes with minimum nbrs
	vector<ar<int,3>> move;
	for(int i = 0; i < 8; ++i) {
		int tr = r + r8[i];
		int tc = c + c8[i];
		if(tr>=0 && tr<N && tc>=0 && tc<N && b[tr][tc]==-1) 
			move.pb({cntnbr(tr, tc), tr, tc}); 
	}
	
	sort(all(move)); // sort according to nbrs asc
	for(int i = 0; i < move.size(); ++i) {
		int tr = move[i][1];
		int tc = move[i][2];
		if(tr>=0 && tr<N && tc>=0 && tc<N && b[tr][tc]==-1) {
			b[tr][tc] = m+1;
			if(dfs(tr, tc, m+1)) return true;
			b[tr][tc] = -1;
		} 
	}
	return false;	
