...
void solve()
{
	...
	d[0][1] = 0;
	pq.push({0, 1, 0}); // {dist, node, discount used ? }
	while(!pq.empty()) {
		auto tp = pq.top(); pq.pop();
		if(d[tp[2]][tp[1]] < tp[0]) continue;
		for(auto &v : g[tp[1]]) {
			int newd = v.second + tp[0];
			if(newd < d[tp[2]][v.first]) {
				d[tp[2]][v.first] = newd;
				pq.push({newd, v.first, tp[2]});
			}
			if(tp[2] == 0) {
				newd = v.second/2 + tp[0];
				if(newd < d[1][v.first]) {
					d[1][v.first] = newd;
					pq.push({newd, v.first, 1});
				}
			}
		}
	}
