...
struct segTree
{
    ...
    node OUT_OF_RANGE = { INT_MIN, INT_MIN, INT_MIN, INT_MIN }; // For Range Query Only
    int LAZY_DEFAULT = 0;
    ...
    void merge(node &cur, node &l, node &r)
    {
        ...
        if(l == OUT_OF_RANGE ) {
            ...
            cur.res = max(max(l.res, r.res), l.suf + r.pre);
        }
    }
    ...
    void propogate(int v, int tl, int tr)
    {
        ...
        if (clazy[v] == true)
        {
            ...
            tree[v].sum = ( lazy[v] );
            tree[v].pre = tree[v].suf = tree[v].res = max(lazy[v], (int)0);
    ...
    void Build(int v, int tl, int tr, vector<int> &a)
    {
        if (tl == tr)
        {
            // complete here
            tree[v].sum = a[tl];
            tree[v].pre = tree[v].suf = tree[v].res = max(a[tl], (int)0);
        }
        else
        {
            int tmid = (tl + tr) / 2;
